...
class LogisticRegressionLayer(Layer):
    ...
    def crossentropy(self, y):
        ...
        """
        return T.mean(-y * T.log(T.flatten(self.p_y_given_x)) \
                - (1 - y) * T.log(T.flatten(self.p_y_given_x)))
...
class MLP:
    ...
    def __init__(self, input, n_in, n_hidden, n_out, rng=None, is_binary=False):
        ...
        if DEBUGGING:
            ...
            self.input.tag.test_value = numpy.random.rand(1800, 704)
    ...
    def train(self,
        ...
        if labels is None:
            raise Exception("Post-training can not start without training class-membership probabilities.")

        self.learning_rate = learning_rate
        
        train_set_x = self._shared_dataset(data, name="training_set")
        train_set_y = T.cast(self._shared_dataset(labels, name="train_labels"), 'int32')

        train_costs_file = self.out_dir + "post_train_costs_batch_size_" + str(batch_size) + "lrate_" + str(self.learning_rate)+ ".npy"
        ...
        if save_costs_file:
            numpy.save(train_costs_file, costs)
        return costs
    ...
    def test(self,
        ...
        test_set_x = self._shared_dataset(data)
        test_set_y = labels
        test_costs_file = self.out_dir + "post_test_costs_batch_size_" + str(batch_size) + "lrate_" + str(self.learning_rate)+ ".npy"
        ...
        # compiling a Theano function `test_model` that returns the cost, butx
        ...
        if save_costs_file:
            numpy.save(test_costs_file, test_losses)
...
if __name__ == "__main__":
    ...
    ncols = 704
    nrows = 1800
    data = numpy.random.rand(nrows, ncols)
    labels = numpy.ones(nrows)
    bin_mlp = MLP(x, n_in=704, n_out=1, n_hidden=400, is_binary=True)
    bin_mlp.train(data=data, labels=labels, batch_size=100)
